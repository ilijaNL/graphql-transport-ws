...
export const emptySubscribe: ServerOptions<GraphqlProtocol>['createSubscription'] =
  () => {
    return {
...
async function getValue(
  emit: (message: GraphqlProtocol['ExecutionResult']) => Promise<void>,
) {
  await emit({ data: { getValue: 'value' } });
}

export const simpleSubscribe: ServerOptions<GraphqlProtocol>['createSubscription'] =
  ({ message }) => {
    ...
    return {
      ...
      start: () =>
        Promise.resolve<GraphqlProtocol['ErrorPayload']>([
